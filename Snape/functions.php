...
function snape_theme_scripts() {  
    ...
    if ( !is_admin() ) {  
        ...
        wp_enqueue_style( 'fontawesome', $dir . '/css/font-awesome.min.css', array(), '4.7.1');
        wp_enqueue_style( 'animate', $dir . '/css/animate.min.css', array(), SNAPE_VERSION);
        wp_enqueue_style( 'layer', $dir . '/css/layer.min.css', array(), SNAPE_VERSION);
...
function snape_wp_title( $title, $sep ) {
    ...
    if ( $paged >= 2 || $page >= 2 )
       $title = "$title $sep " . sprintf( __( 'Page %s', 'snape' ), max( $paged, $page ) );
    return $title;
}
...
function snape_options_menu_filter( $menu ) {
    $menu['mode'] = 'menu';
    $menu['page_title'] = '主题设置';
    $menu['menu_title'] = '主题设置';
    $menu['menu_slug'] = 'snape';
    return $menu;
}
...
function snape_blog_thumbnail() {    
    global $post;  
    $img_id = get_post_thumbnail_id();
    $img_url = wp_get_attachment_image_src($img_id,'snape-thumb');
    $img_url = $img_url[0];
...
function snape_banner(){
    if( !$output = get_option('snape_banners') ){
        $output = '';
        $snape_banner_on = snape_option("snape_banner") ? snape_option("snape_banner") : 0;
        if($snape_banner_on){
            for($i=1; $i<6; $i++){
                $snape_banner{$i} = snape_option("snape_banner{$i}") ? snape_option("snape_banner{$i}") : "";
                $snape_banner_url{$i} = snape_option("snape_banner_url{$i}") ? snape_option("snape_banner_url{$i}") : "";
                if($snape_banner{$i} ){
                    $banners[] = $snape_banner{$i};
                    $banners_url[] = $snape_banner_url{$i};
                }
            }
            $count = count($banners);
            ...
            update_option('snape_banners', $output);
        }
    }
    echo $output;
}
...
add_filter('excerpt_length', 'snape_excerpt_length');
function snape_excerpt_more($more) {
    return '……';
}
...
add_action('wp_head', 'snape_set_post_views');
function snape_get_post_views($before = '', $after = '', $echo = 1)
{
  global $post;
  $post_ID = $post->ID;
...
add_action('wp_ajax_nopriv_love', 'snape_love');
...
add_filter('admin_footer_text', 'snape_admin_footer_text');