...
// 熊掌号 是否启用
function tb_xzh_on(){
	if( cs_get_option('xzh_on') && cs_get_option('xzh_appid') && wp_is_mobile() ){
		return true;
	}
	return false;
}
...
function tb_xzh_is_original(){
	...
	if( cs_get_option('xzh_on') && $isoriginal ){
		return true;
	}
	return false;
}
...
function tb_xzh_head_var(){
	echo (tb_xzh_on()&&(cs_get_option('xzh_render_head')||cs_get_option('xzh_render_body')||cs_get_option('xzh_render_tail'))) ? '<script src="//msite.baidu.com/sdk/c.js?appid='. cs_get_option('xzh_appid') .'"></script>' : '';
}

// 熊掌号 粉丝关注 吸顶bar
function tb_xzh_render_head(){
	echo (tb_xzh_on()&&cs_get_option('xzh_render_head')) ? "<div class='xzh-render-head'><script>cambrian.render('head')</script></div>" : '';
}

// 熊掌号 粉丝关注 文章段落间bar
function tb_xzh_render_body(){
	echo (tb_xzh_on()&&cs_get_option('xzh_render_body')) ? "<div class='xzh-render-body'><script>cambrian.render('body')</script></div>" : '';
}

// 熊掌号 粉丝关注 底部bar
function tb_xzh_render_tail(){
	echo (tb_xzh_on()&&cs_get_option('xzh_render_tail')) ? "<div class='xzh-render-tail'><script>cambrian.render('tail')</script></div>" : '';
}
...
function tb_xzh_jsonld() {
	if ( cs_get_option('xzh_on') && cs_get_option('xzh_appid') && ((is_single()&&cs_get_option('xzh_jsonld_single')) || (is_page()&&cs_get_option('xzh_jsonld_page'))) ){
		echo '<script type="application/ld+json">
    {
        "@context": "https://ziyuan.baidu.com/contexts/cambrian.jsonld",
        "@id": "'.get_the_permalink().'",
        "appid": "'.cs_get_option('xzh_appid').'",
...
function tb_xzh_post_imgs(){
	if( cs_get_option('xzh_jsonld_img') ) return '';
	global $post;
	$src = '';
...
function tb_xzh_post_to_baidu() {
	if( cs_get_option('xzh_on') && cs_get_option('xzh_appid') && cs_get_option('xzh_post_on') && cs_get_option('xzh_post_token') ){
	    global $post;
	    ...
	    if( 'publish' !== $post->post_status && $plink ){
		    ...
		    $urls = array();
		    $urls[] = $plink;
			$api = 'http://data.zz.baidu.com/urls?appid='. cs_get_option('xzh_appid') .'&token='. cs_get_option('xzh_post_token') .'&type=realtime';
			if( $isoriginal ){
		    	$api .= ',original';
		    }
			$ch = curl_init();
