...
if ( $user->ID ) {
	...
	if ( get_option('comment_registration') || 'private' == $status )
		err('<i class="iconfont icon-information-outline"></i>对不起，您必须登录后才能发表评论。'); // 将 wp_die 改为错误提示
}
...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		err('<i class="iconfont icon-information-outline"></i>提示：必须填写昵称及邮件。'); // 将 wp_die 改为错误提示
	elseif ( !is_email($comment_author_email))
		err('<i class="iconfont icon-information-outline"></i>提示：请输入一个有效的电子邮件地址。');// 将 wp_die 改为错误提示
}

if ( '' == $comment_content )
	err('<i class="iconfont icon-information-outline"></i>提示：请输入评论内容。'); // 将 wp_die 改为错误提示
...
if ( $wpdb->get_var($dupe) ) {
    err('<i class="iconfont icon-information-outline"></i>貌似您已发表过重复的评论!');
}
...
if ( $flood_die ) {
    err('<i class="iconfont icon-information-outline"></i>您发表评论也太快了');
	}
}
...
if ( $edit_id ) {
    ...
    if( ihacklog_user_can_edit_comment($commentdata,$comment_id) ){
        ...
        err( '<i class="iconfont icon-information-outline"></i>您没有更改此评论的权限!' );
    }
	}else{
...
?>
	...
	<div class="comment-author vcard">
		...
		<span class="comment-meta commentmetadata">
			...
			<?php printf('%1$s at %2$s', get_comment_date( 'Y年m月d日' ),  get_comment_time() ); ?></a>
		</span>
	</div>
	<?php comment_text(); ?>
	<?php if ( $comment->comment_approved == '0' ) : ?>
		<div class="comment-awaiting-moderation"><i class="iconfont icon-information-outline"></i>您的评论正在等待审核！</div>
	<?php endif; ?>